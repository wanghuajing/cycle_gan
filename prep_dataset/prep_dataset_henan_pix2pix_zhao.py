...
def make_cropped_dataset():
    ...
    P_SIZE = (512, 512)  # 裁片大小
    OVERLAP = 256  # 裁片位移量
    src_root_dir = '/media/zhao/HD1/data/ai-postprocess/mammo300_png/full'
    res_root_dir = '/media/zhao/HD1/data/ai-postprocess/mammo300_png/'
    res_root_dir = os.path.join(res_root_dir, 'patch_%dx%d_o%d') % (P_SIZE[0], P_SIZE[1], OVERLAP)
    os.makedirs(res_root_dir, exist_ok=True)
    proc_paths = glob(os.path.join(src_root_dir, 'proc', '*.png'))
    for proc_path in tqdm(proc_paths):
        basename = os.path.basename(proc_path)
        basename = basename.rstrip('.png')
        os.makedirs(os.path.join(res_root_dir, 'proc', basename), exist_ok=True)
        os.makedirs(os.path.join(res_root_dir, 'od', basename), exist_ok=True)
        im_proc = cv2.imread(proc_path, -1)
        od_path = os.path.join(src_root_dir, 'od', basename + '.png')
        im_od = cv2.imread(od_path, -1)
        im_c = np.stack((im_proc, im_od), axis=2)
        im_c = im_c[20:-20, :, :]

        patchs = patchify(im_c, (P_SIZE[0], P_SIZE[1], 2), OVERLAP)
        ...
        for idx in range(patchs.shape[0]):
            ...
            if np.mean((im_patch[:, :, 1] > 0).astype(np.float32)) < 0.3:
                continue
            cv2.imwrite(os.path.join(res_root_dir, 'proc', basename, basename + '_%03d' % (idx) + '.png'),
                        im_patch[:, :, 0])
            cv2.imwrite(os.path.join(res_root_dir, 'od', basename, basename + '_%03d' % (idx) + '.png'),
                        im_patch[:, :, 1])
    return
...
def low2high_1():
    ...
    src_dir = '/media/zhao/HD1/data/mammo300/all/add_256'
    res_dir = '/media/zhao/HD1/data/mammo300/all/add'
    paths = glob(os.path.join(src_dir, '*.png'))
...
def make_cropped_dataset_add():
    ...
    P_SIZE = (512, 512)  # 裁片大小
    OVERLAP = 256  # 裁片位移量
    src_root_dir_raw = '/media/zhao/HD1/data/mammo300/all/raw'
    src_root_dir_proc = '/media/zhao/HD1/data/mammo300/all/proc'
    src_root_dir_add = '/media/zhao/HD1/data/mammo300/all/add'
    res_root_dir = '/media/zhao/HD1/data/mammo300/all/crop'
    res_root_dir = os.path.join(res_root_dir, 'patch_%dx%d_o%d') % (P_SIZE[0], P_SIZE[1], OVERLAP)
    os.makedirs(res_root_dir, exist_ok=True)
    proc_paths = glob(os.path.join(src_root_dir_proc, '*.png'))
    for proc_path in tqdm(proc_paths):
        basename = os.path.basename(proc_path)
        basename = basename.rstrip('.png')
        os.makedirs(os.path.join(res_root_dir, 'proc', basename), exist_ok=True)
        os.makedirs(os.path.join(res_root_dir, 'raw', basename), exist_ok=True)
        os.makedirs(os.path.join(res_root_dir, 'add', basename), exist_ok=True)
        im_proc = cv2.imread(proc_path, -1)
        od_path = os.path.join(src_root_dir_raw, basename + '.png')
        im_od = cv2.imread(od_path, -1)
        add_path = os.path.join(src_root_dir_add, basename + '.png')
        ...
        for idx in range(patchs.shape[0]):
            ...
            cv2.imwrite(os.path.join(res_root_dir, 'proc', basename, basename + '_%03d' % (idx) + '.png'),
                        im_patch[:, :, 0])
            cv2.imwrite(os.path.join(res_root_dir, 'raw', basename, basename + '_%03d' % (idx) + '.png'),
                        im_patch[:, :, 1])
            cv2.imwrite(os.path.join(res_root_dir, 'add', basename, basename + '_%03d' % (idx) + '.png'),
                        im_patch[:, :, 2])
    return
